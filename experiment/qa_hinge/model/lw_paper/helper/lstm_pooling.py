...
from experiment.qa_hinge.model.lw_paper.helper.pooling_helper import non_zero_tokens, attention_softmax


class LSTMPooling(ComponentBase):
    def __init__(self, config, config_global, logger):
        super(LSTMPooling, self).__init__(config, config_global, logger)
    ...
    def positional_weighting(self, raw_representation, indices, item_type, apply_softmax=True):
        ...
        with tf.variable_scope('positional_weighting_lstm_{}'.format(item_type), reuse=re_use):
            ...
            lstm_output = tf.concat(lstm_outputs, 2)
