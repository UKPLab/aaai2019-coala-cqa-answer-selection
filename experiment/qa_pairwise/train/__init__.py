...
class QABatchedTrainingPairwise(QATraining):
    """This is a simple training method that runs over the training data in a linear fashion, just like in keras."""

    def __init__(self, config, config_global, logger):
        super(QABatchedTrainingPairwise, self).__init__(config, config_global, logger)
    ...
    def start(self, model, data, sess, evaluation):
        super(QABatchedTrainingPairwise, self).start(model, data, sess, evaluation)
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            if model.special_print:
                self.logger.debug('Special tensors:')
                spec_values = sess.run(list(model.special_print.values()), feed_dict={model.dropout_keep_prob: 1.0})
                for k, v in zip(model.special_print.keys(), spec_values):
            ...
            for _ in bar(range(int(self.get_n_batches()))):
                ...
                train_questions, train_answers, train_labels = self.get_next_batch(model, data, sess)
                ...
                _, loss, loss_individual, summary = sess.run(
                    ...
                    feed_dict={
                        ...
                        model.input_answer: train_answers,
                        model.input_label: train_labels,
